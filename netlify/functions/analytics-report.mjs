...
export default async (request,context)=>{
  ...
  if(!verifyAdminToken(getBearerToken(request),process.env.HOTLINE_ADMIN_SECRET))return jsonResponse(401,{ok:false,message:'Your admin session has expired. Log in again to view analytics.'});
  ...
  try{const {records,...coverage}=await readEvents(analyticsStore(context),shiftDay(range.previousStart,-1),shiftDay(range.end,1));return jsonResponse(200,{...buildReport(records,range,filters),coverage});}
