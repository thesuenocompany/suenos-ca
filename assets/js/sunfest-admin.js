(function(){
  ...
  function openPreview(id){const adminToken=token();if(!adminToken){status.textContent='Your admin session has expired. Please log in again.';return;}window.open(`/en-ca/?campaign-preview=1&campaign-id=${encodeURIComponent(id)}#campaign-admin-token=${encodeURIComponent(adminToken)}`,'_blank');}
