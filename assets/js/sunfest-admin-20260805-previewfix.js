(function(){
  ...
  function openPreview(id){
    ...
    try{
      localStorage.setItem(PREVIEW_KEY,JSON.stringify({id,overlay,createdAt:Date.now()}));
    }catch(error){
      status.textContent='The preview draft could not be prepared in this browser.';
      return;
    }
    window.open(`/en-ca/?campaign-preview=1&campaign-id=${encodeURIComponent(id)}#campaign-admin-token=${encodeURIComponent(adminToken)}`,'_blank');
  }
