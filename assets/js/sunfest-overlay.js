(function(){
  ...
  if(isPreview){
    const match=location.hash.match(/(?:^#|&)campaign-admin-token=([^&]+)/)||location.hash.match(/(?:^#|&)sunfest-admin-token=([^&]+)/);
    if(match){try{token=decodeURIComponent(match[1]);sessionStorage.setItem(TOKEN_KEY,token);}catch{}history.replaceState(null,'',location.pathname+location.search);}
  }
  ...
  fetch('/api/campaign-overlays',{headers,cache:'no-store'})
    ...
    .then(data=>{
      const overlay=isPreview?(data.overlays||[]).find(item=>item.id===previewId)||data.activeOverlay||data.overlays?.[0]:data.overlay;
      if(!overlay){if(isPreview)showPreviewError('No saved campaign was available for preview.');return;}
