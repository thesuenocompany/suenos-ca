...
const mobileHero=ASSET+'sunfest-contest-hero-mobile.png';
...
const MAILCHIMP_FRAME='suenos-mailchimp-contest-submit';
const latestEligibleBirthDate=age=>{const d=new Date();d.setFullYear(d.getFullYear()-Math.max(0,Number(age)||0));return d.toISOString().slice(0,10)};
const isOldEnough=(value,age)=>{if(!/^\d{4}-\d{2}-\d{2}$/.test(String(value||'')))return false;const [y,m,d]=value.split('-').map(Number);const born=new Date(Date.UTC(y,m-1,d));if(born.getUTCFullYear()!==y||born.getUTCMonth()!==m-1||born.getUTCDate()!==d)return false;const today=new Date(),cutoff=new Date(today.getFullYear()-Math.max(0,Number(age)||0),today.getMonth(),today.getDate());return born<=cutoff;};
...
const submitSocietySignup=payload=>{if(!payload?.marketingConsent||!payload?.email||!payload?.birthDate)return false;const parts=String(payload.birthDate).split('-');if(parts.length!==3)return false;ensureMailchimpFrame();const signup=document.createElement('form');signup.method='post';signup.action=MAILCHIMP_ACTION;signup.target=MAILCHIMP_FRAME;signup.hidden=true;const fields={EMAIL:payload.email,'MMERGE5[day]':parts[2],'MMERGE5[month]':parts[1],'MMERGE5[year]':parts[0],'gdpr[42876]':'Y','b_54180635caa88c03ba452c3bc_db7e420555':'',subscribe:'Subscribe'};Object.entries(fields).forEach(([name,value])=>{const input=document.createElement('input');input.type='hidden';input.name=name;input.value=String(value||'');signup.appendChild(input)});document.body.appendChild(signup);signup.submit();setTimeout(()=>signup.remove(),1500);return true;};
...
window.renderPhotoStoryContest=async function(page,c,track){
  ...
  const minimumAge=Math.max(1,Number(c.minimum_age)||19);
  let entries=[];
  ...
  const rulesTrigger=window.ContestRulesModal?.triggerHtml(c,'Full Rules','contest-rules-trigger ps-prize-link')||`<button class="contest-rules-trigger ps-prize-link" type="button" data-contest-rules-open>Full Rules</button>`;
  ...
  page.innerHTML=`<article class="ps-shell" style="--ps-paper:url('${paperBg}')">
    ...
    <section id="enter" class="ps-entry"><div class="ps-entry-intro"><p class="ps-kicker">ENTER TO WIN</p><h2>Put your memory in the album.</h2><p>${esc(c.intro_copy||'Upload a favourite Sunfest photo and tell us what was happening.')}</p><ul><li>Photo entries are reviewed before appearing publicly.</li><li>Your email is never shown in the gallery.</li><li>No purchase or festival ticket is required.</li></ul>${!active?`<div class="ps-status">${coming?`Entries open ${esc(fmtDate(c.start_at,c.timezone,{dateStyle:'long',timeStyle:'short'}))}.`:'Entries are closed.'}</div>`:''}</div>
      <div class="ps-entry-form">${active?`<div id="ps-errors" class="ps-errors" role="alert"></div><form id="ps-form" novalidate>
        ...
        <div class="full"><label for="ps-birth">Date of birth</label><input id="ps-birth" name="birthDate" type="date" autocomplete="bday" max="${latestEligibleBirthDate(minimumAge)}" required><small class="ps-field-help">Used to confirm you are ${minimumAge}+ and, only when selected, to complete your Sueños Society signup.</small></div>
  ...
  const postEntry=async token=>{
    try{
      ...
      Object.assign(payload,{slug:c.slug,entryType:'photo',birthDate:fd.get('birthDate'),photoAssetKey:uploaded.key,photoMime:uploaded.mime,photoSize:uploaded.size,photoRightsConfirmed:fd.get('photoRightsConfirmed')==='on',photoRightsVersion:c.photo_rights_version||'1.0',ageConfirmed:true,rulesConfirmed:fd.get('rulesConfirmed')==='on',marketingConsent:fd.get('marketingConsent')==='on',turnstileToken:token,utmSource:p.get('utm_source'),utmMedium:p.get('utm_medium'),utmCampaign:p.get('utm_campaign'),utmContent:p.get('utm_content'),referrer:document.referrer});
  ...
  form.addEventListener('submit',async e=>{
    ...
    if(!birthDate)errors.push('Enter your date of birth.');
    else if(!isOldEnough(birthDate,minimumAge))errors.push(`You must be at least ${minimumAge} years old to enter.`);
