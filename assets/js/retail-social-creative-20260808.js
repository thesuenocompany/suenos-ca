...
async function api(action,{method='GET',body}={}){
  ...
  const r=await fetch(`/api/contest-admin?action=${encodeURIComponent(action)}&id=${encodeURIComponent(id)}`,{method,headers:{authorization:`Bearer ${token()}`,...(body?{'content-type':'application/json'}:{})},body:body?JSON.stringify(body):undefined,cache:'no-store'});
...
function applyConfig(c={}){config={creatives:ensureFour(c.creatives||[]),sourceContestId:c.source_contest_id||'',sourceName:c.source_internal_name||'',inherited:Boolean(c.inherited),retailerName:c.request_retailer_name||$('contest-retailer-name')?.value||'',retailerCode:c.request_retailer_code||$('contest-retailer-code')?.value||'',retailerLogoUrl:c.request_retailer_logo_url||$('contest-retailer-logo')?.value||'',slug:c.request_slug||$('contest-slug')?.value||''};renderSlots();}
...
async function renderCreative(index){
 const c=ensureFour(config.creatives)[index];if(!c.url)throw new Error(`Creative ${index+1} has no image.`);if(!c.box)throw new Error(`Creative ${index+1} needs a retailer-brand placement box on the master.`);
 ...
 const logoUrl=config.retailerLogoUrl||$('contest-retailer-logo')?.value||'';
 if(logoUrl){
   try{const logo=await loadImage(logoUrl);const pad=Math.max(6,Math.min(w,h)*.06),maxW=w-pad*2,maxH=h-pad*2,scale=Math.min(maxW/logo.naturalWidth,maxH/logo.naturalHeight,1e6),dw=logo.naturalWidth*scale,dh=logo.naturalHeight*scale;ctx.drawImage(logo,x+(w-dw)/2,y+(h-dh)/2,dw,dh);}catch{await drawName(ctx,x,y,w,h);}
 }else await drawName(ctx,x,y,w,h);
...
async function downloadZip(){setStatus('Generating retailer social ZIP…');const files=[];for(let i=0;i<4;i++){if(!ensureFour(config.creatives)[i].url)continue;files.push({name:filename(i),blob:await renderCreative(i)});}if(!files.length)throw new Error('The master campaign has no social creatives yet.');const zip=await makeZip(files),a=document.createElement('a'),u=URL.createObjectURL(zip);a.href=u;a.download=`suenos-${slugify(config.retailerName||config.slug)}-social-pack.zip`;a.click();setTimeout(()=>URL.revokeObjectURL(u),1500);setStatus(`Social pack downloaded for ${config.retailerName}.`);}
