(()=>{
  ...
  const defs=[
    ...
    {key:'prize',label:'Prize',short:'What they can win',title:'Define the prize',desc:'Be precise about what is included, what is excluded, and the approximate retail value.'},
    {key:'timing',label:'Dates',short:'Start, end and draw',title:'Contest dates',desc:'Set the exact opening, closing and draw dates. These dates control when the contest accepts entries.'},
  ...
  const cards={
    ...
    schedule:card('timing','Contest timing','These are the dates that control the contest. All times use the time zone selected in Start.'),
  ...
  function completion(){
    ...
    const checks={
      ...
      prize:Boolean(value('contest-prize-title','')),
      timing:Boolean(value('contest-start','')&&value('contest-close','')&&value('contest-draw','')),
  ...
  function review(){
    ...
    const c=completion();
    const items=[['start','Names and URL are complete'],['page','Public page has a headline or hero'],['prize','Prize details are complete'],['timing','Start, end and draw dates are set'],['entry','Eligibility and age requirements are set'],['rules','Official Rules are configured']];
