...
window.renderRetailContest=async function(page,c,track){
  ...
  const heroMarkup=c.desktop_hero_url?`<div class="rc-hero-blur" aria-hidden="true" style="background-image:url('${esc(c.desktop_hero_url)}')"></div><picture>${c.mobile_hero_url?`<source media="(max-width:720px)" srcset="${esc(c.mobile_hero_url)}">`:''}<img src="${esc(c.desktop_hero_url)}" alt="${esc(c.hero_alt||c.public_name)}" style="object-position:${esc(c.hero_object_position||'50% 50%')}"></picture>`:`<div class="rc-hero-fallback" aria-hidden="true"><span>Paradise is a state of Sueños.</span></div>`;
  ...
  page.innerHTML=`<article class="rc-shell">
    <section class="rc-hero">${heroMarkup}</section>
  ...
  const submitEntry=async token=>{
    ...
    try{
      ...
      form.outerHTML=`<div class="rc-success"><p class="rc-kicker">ENTRY RECEIVED</p><h2>${esc(d.confirmation?.heading||'You’re in. Paradise may be calling.')}</h2><p>${esc(d.confirmation?.message||'Your entry has been received.')}</p>${receiptCopy}${subscribed?'<p>Check your inbox to confirm your Sueños Society signup.</p>':''}</div>`;
    }catch(err){
